...
—◉ *Contacto para apelaciones:* wa.me/5217776120198
╚═════════════════════╝
               `.trim();
              m.reply(messageText);
  ...
  for (const nk of callUpdate) {
    if (nk.isGroup == false) {
      if (nk.status == 'offer') {
        ...
        const vcard = `BEGIN:VCARD\nVERSION:3.0\nN:;@robledojuan10;;;\nFN:@robledojuan10\nORG:@robledojuan10\nTITLE:\nitem1.TEL;waid=5219992095479:+521 999 209 5479\nitem1.X-ABLabel:@robledojuan10\nX-WA-BIZ-DESCRIPTION:[❗] ᴄᴏɴᴛᴀᴄᴛᴀ ᴀ ᴇsᴛᴇ ɴᴜᴍ ᴘᴀʀᴀ ᴄᴏsᴀs ɪᴍᴘᴏʀᴛᴀɴᴛᴇs.\nX-WA-BIZ-NAME:@robledojuan10\nEND:VCARD`;
        await mconn.conn.sendMessage(nk.from, {contacts: {displayName: '@robledojuan10', contacts: [{vcard}]}}, {quoted: callmsg});
...
global.dfail = (type, m, conn) => {
  ...
  const prep = generateWAMessageFromContent(m.chat, {extendedTextMessage: {text: msg, contextInfo: {externalAdReply: {title: '𝐀𝐋𝐄𝐑𝐓𝐀', body: 'Manuel R', thumbnail: imagen1, sourceUrl: '@robledojuan10'}}}}, aa);
